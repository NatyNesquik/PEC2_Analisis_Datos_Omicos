...
* el gen se expresa con el doble de transcripciones que elotros gen.
...
```{r, echo=FALSE}
cat("Se comprueba que el Objeto dispone de un total de:" , nrow(dds),"genes")
```


Se comprueba que el Objeto dispone de los 19.471 genes en las 30 muestras
...
Guardaremos en diferentes tablas los resultados de genes expresados diferencialmente cuyo p-valor sea inferior a 0,05 en las comparativas ELI-NIT y ELI-SFI.

```{r}
dif_exp_ELI_NIT <- topTab_ELI_NIT[1:5441,]
head(dif_exp_ELI_NIT)
```

```{r}
dif_exp_ELI_SFI <- topTab_ELI_SFI[1:2725,]
head(dif_exp_ELI_SFI)
```
